...
public class P2nX {
	...
	public static void main(String[] args) {
		...
		try {
			...
			if (genero == 'M') {
				primeiraPessoa = new Homem(nome, sobrenome, dia, mes, ano, peso, altura);
			} else if (genero == 'F') {
				primeiraPessoa = new Mulher(nome, sobrenome, dia, mes, ano, peso, altura);
			} else {
				primeiroSucesso = false;
		...
		if (primeiroSucesso) {
			...
			while (index < numElementos) {
				try {
					...
					if (genero == 'M') {
						pessoas[index] = new Homem(nome, sobrenome, dia, mes, ano, peso, altura);
					} else if (genero == 'F') {
						pessoas[index] = new Mulher(nome, sobrenome, dia, mes, ano, peso, altura);
					} else {
